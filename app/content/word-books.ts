...
export function resolveRuntimeWordId(wordId: string) {
  ...
  if (B1_VALID_IDS.has(wordId)) return wordId;
  return undefined;
}

export function wordBookForWordId(wordId: string): WordBookId | undefined {
  if (B1_VALID_IDS.has(wordId)) return "b1";
...
export function wordBookIdForLevel(level: LevelId): WordBookId {
  return level.toLocaleLowerCase("en-US") as WordBookId;
}
